...
int c[10] = {4, 8, 2, 6, 1}; /* c[i] is the *COST* of the ith object; i.e. what
                YOU PAY to take the object */
int v[10] = {12, 32, 40, 30, 50}; /* v[i] is the *VALUE* of the ith object; i.e.
...
void simple_fill() {
    ...
    while (cur_w > 0) { /* while there's still room*/
        ...
        tot_v += v[maxi];
        if (cur_w >= 0)
            printf("Item %d Amount %d$ Profit %d\n", maxi + 1, v[maxi], c[maxi]);
        else {
